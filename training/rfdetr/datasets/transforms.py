...
def resize(image, target, size, max_size=None):
    ...
    def get_size_with_aspect_ratio(image_size, size, max_size=None):
        ...
        # Ensure both dimensions are divisible by 32 for backbone compatibility
        ow = (ow // 32) * 32
        oh = (oh // 32) * 32
...
class AdaptiveNormalize(object):
    """
    Adaptive normalization that automatically detects image channels and applies appropriate normalization.
    - Single channel (grayscale): Uses [0.5] mean and [0.5] std
    - Three channels (RGB): Uses ImageNet means and stds
    """
    ...
    def __call__(self, image, target=None):
        ...
        if image.shape[0] == 1:
            ...
            image = F.normalize(image, mean=self.grayscale_mean, std=self.grayscale_std)
        elif image.shape[0] == 3:
            # Three channels (RGB)
            image = F.normalize(image, mean=self.rgb_mean, std=self.rgb_std)
        else:
            raise ValueError(f"Unsupported number of channels: {image.shape[0]}. Expected 1 (grayscale) or 3 (RGB).")
