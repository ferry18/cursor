...
class CocoDetection(torchvision.datasets.CocoDetection):
    ...
    def _load_image(self, id: int):
        """Override to preserve monochrome images instead of forcing RGB conversion."""
        path = self.coco.loadImgs(id)[0]["file_name"]
        image = torchvision.datasets.folder.default_loader(os.path.join(self.root, path))
        
        # Preserve single-channel images instead of forcing RGB conversion
        if image.mode in ['L', 'LA']:
            return image.convert('L')  # Keep as grayscale
        elif image.mode == 'RGB':
            return image  # Keep as RGB
        else:
            return image.convert('L')  # Convert others to grayscale
...
def make_coco_transforms(image_set, resolution, multi_scale=False, expanded_scales=False, skip_random_resize=False, patch_size=16, num_windows=4):
    ...
    # Use adaptive normalization that works for both RGB and grayscale
    normalize = T.Compose([
        T.ToTensor(),
        T.AdaptiveNormalize()  # Will automatically detect channels and normalize appropriately
    ])
    ...
    if image_set == 'train':
        return T.Compose([
            ...
            T.RandomSelect(
                T.RandomResize(scales, max_size=resolution),  # Use resolution as max_size for high-res images
                T.Compose([
    ...
    if image_set == 'val':
        return T.Compose([
            T.RandomResize([resolution], max_size=resolution),  # Use resolution as max_size for high-res images
            normalize,
        ])
    if image_set == 'test':
        return T.Compose([
