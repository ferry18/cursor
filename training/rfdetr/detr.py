...
class RFDETR:
    ...
    def predict(
        ...
        **kwargs,
        ...
        (file path, PIL Image, NumPy array, or torch.Tensor). The images can be in
        RGB or grayscale (single-channel) format. If a torch.Tensor is provided, it must already be normalized
        ...
        for img in images:
            ...
            if isinstance(img, str):
                ...
                if img.mode in ['L', 'LA']:
                    ...
                    img = img.convert('L')  # Convert others to grayscale
            ...
            # Allow both single-channel and RGB images
            if img.shape[0] not in [1, 3]:
                raise ValueError(
                    f"Invalid image shape. Expected 1 (grayscale) or 3 (RGB) channels, but got "
            ...
            # Apply adaptive normalization based on number of channels
            if img_tensor.shape[0] == 1:
                # Grayscale normalization
                img_tensor = F.normalize(img_tensor, [0.5], [0.5])
            elif img_tensor.shape[0] == 3:
                # RGB normalization
                img_tensor = F.normalize(img_tensor, self.means, self.stds)
            else:
                raise ValueError(f"Unsupported number of channels: {img_tensor.shape[0]}. Expected 1 (grayscale) or 3 (RGB).")
