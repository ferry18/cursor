...
class Model:
    ...
    def train(self, callbacks: DefaultDict[str, List[Callable]], **kwargs):
        ...
        if hasattr(args, 'dataset_fraction') and args.dataset_fraction is not None and args.dataset_fraction < 1.0:
            # Set random seed for reproducible subset selection
            random.seed(args.seed)
            
            # Calculate subset sizes
            train_size = int(len(dataset_train) * args.dataset_fraction)
            val_size = int(len(dataset_val) * args.dataset_fraction)
            test_size = int(len(dataset_test) * args.dataset_fraction)
            
            # Create random indices for subset selection
            train_indices = random.sample(range(len(dataset_train)), train_size)
            val_indices = random.sample(range(len(dataset_val)), val_size)
            ...
            print(f"Using {args.dataset_fraction*100:.1f}% of dataset:")
            print(f"  Training: {len(dataset_train)} samples (from {len(dataset_train.dataset) if hasattr(dataset_train, 'dataset') else 'unknown'})")
            print(f"  Validation: {len(dataset_val)} samples (from {len(dataset_val.dataset) if hasattr(dataset_val, 'dataset') else 'unknown'})")
            print(f"  Test: {len(dataset_test)} samples (from {len(dataset_test.dataset) if hasattr(dataset_test, 'dataset') else 'unknown'})")
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            # Handle case where COCO evaluation failed
            if "coco_eval_bbox" not in test_stats:
                print("Warning: COCO evaluation failed, using default metrics")
                map_regular = 0.0
                test_stats["coco_eval_bbox"] = [0.0, 0.0]  # Default values
            else:
            ...
            if args.use_ema:
                ...
                # Handle case where COCO evaluation failed for EMA
                if "coco_eval_bbox" not in ema_test_stats:
                    print("Warning: COCO evaluation failed for EMA model, using default metrics")
                    map_ema = 0.0
                    ema_test_stats["coco_eval_bbox"] = [0.0, 0.0]  # Default values
                else:
        ...
        if utils.is_main_process():
            ...
            if best_is_ema:
                results = ema_test_stats.get("results_json", {})
            else:
                results = test_stats.get("results_json", {})
