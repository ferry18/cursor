...
class WindowedDinov2WithRegistersEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor, bool_masked_pos: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if self.config.num_windows > 1:
            ...
            # Validate that dimensions are compatible with windowing
            if num_h_patches % self.config.num_windows != 0:
                raise ValueError(f"Height patches ({num_h_patches}) must be divisible by num_windows ({self.config.num_windows}) for windowed attention. Consider adjusting image height or num_windows.")
            if num_w_patches % self.config.num_windows != 0:
                raise ValueError(f"Width patches ({num_w_patches}) must be divisible by num_windows ({self.config.num_windows}) for windowed attention. Consider adjusting image width or num_windows.")
