...
void GetSpecialObjectValue(const EC_T_BYTE* const pbyValue, const EC_T_WORD wByteLen, EC_T_CHAR* const pStringBuffer, const EC_T_WORD wStringBufferLen, const EC_T_CHAR* const pValueDescription)
{
    switch (wByteLen)
    {
    case 1: OsSnprintf(pStringBuffer, wStringBufferLen, ", %s 0x%02X", pValueDescription, *pbyValue); break;
    case 2: OsSnprintf(pStringBuffer, wStringBufferLen, ", %s 0x%04X", pValueDescription, EC_GET_FRM_WORD(pbyValue)); break;
    case 4: OsSnprintf(pStringBuffer, wStringBufferLen, ", %s 0x%08X", pValueDescription, EC_GET_FRM_DWORD(pbyValue)); break;
    default: OsSnprintf(pStringBuffer, wStringBufferLen, ", %s '-'", pValueDescription); break;
    }
}
...
#define MAX_OBNAME_LEN          ((EC_T_DWORD)100)
    ...
    /* create mailbox transfer object for OD list upload */
    ...
    /* create mailbox transfer object for Object description upload */
    ...
    /* create mailbox transfer object for Entry description upload */
    ...
    dwRes = emCoeGetODListReq(pAppContext->dwInstanceId, pMbxGetODLTfer, dwNodeId, eODListType_ALL, dwTimeout);
    ...
    for (wODListLen = 0, wIndex = 0; wIndex < (pMbxGetODLTfer->MbxData.CoE_ODList.wLen); wIndex++)
    {
        ...
        {
            OsSnprintf(&szLogBuffer[OsStrlen(szLogBuffer)], (EC_T_INT)(LOG_BUFFER_SIZE - OsStrlen(szLogBuffer) - 1), "0x%04X ", pwODList[wODListLen]);
    ...
    if (!bReadingMasterOD)
    {
        ...
        dwRes = emCoeGetODListReq(pAppContext->dwInstanceId, pMbxGetODLTfer, dwNodeId, eODListType_RxPdoMap, dwTimeout);
        if (EC_E_SLAVE_NOT_PRESENT == dwRes)
        {
            dwRetVal = dwRes;
            goto Exit;
        ...
        if (pAppContext->AppParms.dwAppLogLevel >= EC_LOG_LEVEL_INFO)
        {
            ...
            {
                OsSnprintf(&szLogBuffer[OsStrlen(szLogBuffer)], (EC_T_INT)(LOG_BUFFER_SIZE - OsStrlen(szLogBuffer) - 1), "0x%04X ", pMbxGetODLTfer->MbxData.CoE_ODList.pwOdList[wIndex]);
                if (((wIndex+1) % 10) == 0) FlushLogBuffer(pAppContext, szLogBuffer);
            }
            FlushLogBuffer(pAppContext, szLogBuffer);
        }
        ...
        dwRes = emCoeGetODListReq(pAppContext->dwInstanceId, pMbxGetODLTfer, dwNodeId, eODListType_TxPdoMap, dwTimeout);
        if (EC_E_SLAVE_NOT_PRESENT == dwRes)
        {
            dwRetVal = dwRes;
            goto Exit;
    ...
    for (wIndex = 0; (wIndex < wODListLen) && !*pbStopReading; wIndex++)
    {
        ...
        /* get object description */
        dwRes = emCoeGetObjectDescReq(pAppContext->dwInstanceId, pMbxGetObDescTfer, dwNodeId, pwODList[wIndex], dwTimeout);
        if (EC_E_SLAVE_NOT_PRESENT == dwRes)
        {
            dwRetVal = dwRes;
            goto Exit;
        ...
        if (pAppContext->AppParms.dwAppLogLevel >= EC_LOG_LEVEL_INFO)
        {
            EC_T_CHAR szObName[MAX_OBNAME_LEN + 1];

            OsSafeStrcpy(szObName, pMbxGetObDescTfer->MbxData.CoE_ObDesc.pchObName, (EC_T_INT)EC_AT_MOST(pMbxGetObDescTfer->MbxData.CoE_ObDesc.wObNameLen, sizeof(szObName)));

            EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "0x%04X %s: type 0x%04X, code=0x%02x, %s, SubIds=%d",
        ...
        for (wSubIndex = 0; wSubIndex < wSubIndexLimit; wSubIndex++)
        {
            ...
            dwRes = emCoeGetEntryDescReq(pAppContext->dwInstanceId, pMbxGetEntryDescTfer, dwNodeId, pwODList[wIndex], EC_LOBYTE(wSubIndex), byValueInfoType, dwTimeout);
            if (EC_E_SLAVE_NOT_PRESENT == dwRes)
            {
                dwRetVal = dwRes;
                goto Exit;
            ...
            /* display EntryDesc */
            {
                ...
                EC_T_CHAR   szDefaultValue[24];
                EC_T_CHAR   szMinValue[24];
                EC_T_CHAR   szMaxValue[24];
                ...
                {
                    OsSnprintf(&szPdoMapInfo[OsStrlen(szPdoMapInfo)], (EC_T_INT)(sizeof(szPdoMapInfo) - OsStrlen(szPdoMapInfo) - 1), "%s", "-RxPDO");
                }
                if (pMbxGetEntryDescTfer->MbxData.CoE_EntryDesc.bTxPdoMapping)
                {
                    OsSnprintf(&szPdoMapInfo[OsStrlen(szPdoMapInfo)], (EC_T_INT)(sizeof(szPdoMapInfo) - OsStrlen(szPdoMapInfo) - 1), "%s", (pMbxGetEntryDescTfer->MbxData.CoE_EntryDesc.bRxPdoMapping ? "+" : "-"));
                    OsSnprintf(&szPdoMapInfo[OsStrlen(szPdoMapInfo)], (EC_T_INT)(sizeof(szPdoMapInfo) - OsStrlen(szPdoMapInfo) - 1), "%s", "TxPDO");
                }
                if (pMbxGetEntryDescTfer->MbxData.CoE_EntryDesc.byValueInfo & EC_COE_ENTRY_UnitType)
                {
                    GetSpecialObjectValue(&pMbxGetEntryDescTfer->MbxData.CoE_EntryDesc.pbyData[0],
                                          4,
                                          szUnitType,
                                          sizeof(szUnitType) - 1,
                                          "UnitType");
                ...
                {
                    GetSpecialObjectValue(&pMbxGetEntryDescTfer->MbxData.CoE_EntryDesc.pbyData[nDataIdx],
                                          BIT2BYTE(pMbxGetEntryDescTfer->MbxData.CoE_EntryDesc.wBitLen),
                                          szDefaultValue,
                                          sizeof(szDefaultValue) - 1,
                                          "Default");
                ...
                EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "0x%04X:%d %s: data type=0x%04X, bit len=%02d, %s%s%s%s%s%s",
            ...
            if (bPerformUpload)
            {
                ...
                dwRes = emCoeSdoUpload(
                    ...
                    abySDOValue, (EC_T_WORD)EC_AT_MOST(BIT2BYTE(pMbxGetEntryDescTfer->MbxData.CoE_EntryDesc.wBitLen), (EC_T_WORD)CROD_MAXSISDO_SIZE), &dwUploadBytes, dwTimeout, 0);
                if (EC_E_SLAVE_NOT_PRESENT == dwRes)
                {
                    dwRetVal = dwRes;
                    goto Exit;
                ...
                case DEFTYPE_UNSIGNED8:
                    {
                        EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "0x%04X:%d BYTE: 0x%02X",
                            pwODList[wIndex], EC_LOBYTE(wSubIndex), abySDOValue[0]));
                    } break;
                case DEFTYPE_INTEGER16:
                    {
                        EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "0x%04X:%d SI16: %04d",
                ...
                case DEFTYPE_UNSIGNED16:
                    {
                        EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "0x%04X:%d WORD: 0x%04X",
                            pwODList[wIndex], EC_LOBYTE(wSubIndex), EC_GET_FRM_WORD(&abySDOValue[0])));
                    } break;
                case DEFTYPE_INTEGER32:
                    {
                        EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "0x%04X:%d SI32: %08ld",
                            pwODList[wIndex], EC_LOBYTE(wSubIndex), EC_GET_FRM_DWORD(&abySDOValue[0])));
                    } break;
                case DEFTYPE_UNSIGNED32:
                    {
                        EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "0x%04X:%d DWRD: 0x%08lX",
                            pwODList[wIndex], EC_LOBYTE(wSubIndex), EC_GET_FRM_DWORD(&abySDOValue[0])));
                    } break;
                case DEFTYPE_VISIBLESTRING:
                    {
                        abySDOValue[EC_AT_MOST(dwUploadBytes, CROD_MAXSISDO_SIZE - 1)] = '\0';
                        EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "0x%04X:%d STRG: %s",
                ...
                case DEFTYPE_OCTETSTRING:
                    {
                        ...
                        if ((COEOBJID_HISTORY_OBJECT == (pwODList[wIndex])) && (EC_LOBYTE(wSubIndex) > 5))
                        {
                            ...
                            EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "0x%04X:%d DIAG # 0x%lx type <%s> Text 0x%x Time: 0x%x.%x",
                                pwODList[wIndex], EC_LOBYTE(wSubIndex),
                                pDiag->dwDiagNumber,
...
#endif /* INCLUDE_MASTER_OBD */
                        {
                            EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "0x%04X:%d OCTS: %s",
                                pwODList[wIndex], EC_LOBYTE(wSubIndex), (EC_T_CHAR*)abySDOValue));
                        }
                    } break;
                case DEFTYPE_UNSIGNED48:
                    {
                        EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "0x%04X:%d US48: %02X:%02X:%02X:%02X:%02X:%02X",
                            pwODList[wIndex], EC_LOBYTE(wSubIndex),
                            abySDOValue[0],
                            abySDOValue[1],
                            abySDOValue[2],
                ...
                case DEFTYPE_UNSIGNED64:
                    {
                        EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "0x%04X:%d QWRD: 0x%08lX.%08lX\n",
                            pwODList[wIndex], EC_LOBYTE(wSubIndex),
                ...
                default:
                    {
                        ...
                        EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "0x%04X:%d DFLT: \n", pwODList[wIndex], EC_LOBYTE(wSubIndex)));
