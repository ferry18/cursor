...
#define PCAP_RECORDER_BUF_FRAME_CNT 1000
#endif
...
/* MAX_NUMOF_MASTER_INSTANCES defined in EcMaster.h */
...
public:
    ...
    EC_T_DWORD  LogDcm(                         const
                                                EC_T_CHAR*              szFormat,...                );
    EC_T_DWORD  InitLogging(                    EC_T_DWORD              dwInstanceId,
                                                EC_T_WORD               wRollOver,
                                                EC_T_DWORD              dwPrio,
                                                EC_T_CPUSET             CpuSet,
    ...
    EC_T_DWORD  InitLogging(                    EC_T_DWORD              dwInstanceId,
                                                ...
                                                EC_T_DWORD              dwErrMsgBufferSize = DEFAULT_ERR_MSG_BUFFER_SIZE,
                                                EC_T_DWORD              dwDcmMsgBufferSize = DEFAULT_DCM_MSG_BUFFER_SIZE)
                                                { EC_UNREFPARM(dwLogLevel); return InitLogging(dwInstanceId, wRollOver, dwPrio, CpuSet, szFilenamePrefix, dwStackSize, dwLogMsgBufferSize, dwErrMsgBufferSize, dwDcmMsgBufferSize); }
    ...
    /********************************************************************************/
    /** \brief Log message call-back handler for EC_T_LOG_PARMS::pfLogMsg
     *
     * \return N/A
     */
    template <class T>
    static
    EC_T_DWORD LogMsgCallback(struct _EC_T_LOG_CONTEXT* pContext, EC_T_DWORD dwLogMsgSeverity, const EC_T_CHAR* szFormat, ...)
    {
        ...
        else
        {
            ...
            {
                EC_VASTART(vaArgs, szFormat);
                dwRetVal = ((T*)pContext)->InsertNewMsgVa(((T*)pContext)->m_pErrorMsgBufferDesc, dwLogMsgSeverity, szFormat, vaArgs);
                EC_VAEND(vaArgs);
            }
            EC_VASTART(vaArgs, szFormat);
            dwRetVal = ((T*)pContext)->InsertNewMsgVa(((T*)pContext)->m_pAllMsgBufferDesc, dwLogMsgSeverity, szFormat, vaArgs);
            EC_VAEND(vaArgs);
        }
        return dwRetVal;
    }
...
private:
    ...
    } m_aData[3]; /* 0: EC-Master / EC-Monitor, 1: EC-Simulator. 2: Eap */
...
#elif (defined INCLUDE_EC_MONITOR)
        emonLogFrameEnable(m_dwInstanceId, StaticFrameHandler, this);
#elif (defined INCLUDE_EC_MASTER)
...
public:
    ...
    virtual EC_T_VOID Reset(EC_T_DWORD dwInstanceId = 0xFFFF) /* EC_OVERRIDE */
    {
    ...
    /** \brief parses EtherCAT cmds. dwLogFlags: see EC_LOG_FRAME_FLAG_... */
    virtual EC_T_VOID FrameHandler(EC_T_DWORD dwLogFrameFlags, EC_T_DWORD dwFrameSize, EC_T_BYTE* pbyFrame) /* EC_OVERRIDE */;

    /** \brief EtherCAT cmd handler called by CFrameSpy::FrameHandler */
    virtual EC_T_VOID DatagramHandler(EC_T_DWORD dwLogFrameFlags, EC_T_DWORD dwFrameSize, EC_T_BYTE* pbyFrame, struct TETYPE_EC_CMD_HEADER* pEcCmdHdr, EC_T_WORD wCmdLen) /* EC_OVERRIDE */
    {
        EC_UNREFPARM(dwLogFrameFlags); EC_UNREFPARM(dwFrameSize); EC_UNREFPARM(pbyFrame); EC_UNREFPARM(pEcCmdHdr); EC_UNREFPARM(wCmdLen);
    }
...
public:
    ...
    virtual ~CPcapFileWriter()
    { 
        Close();
        SafeOsFree(m_szFileName);
    }
...
public:
    ...
    virtual EC_T_VOID Close(EC_T_VOID) EC_OVERRIDE
    {
...
/** \brief Application logging example class, e.g. print to debug output window in Visual Studio
    
    Integration Example in main():

        CMyAppLogging oLogging;
        ...
        AppContext.LogParms.pfLogMsg = CAtEmLogging::LogMsgCallback<CMyAppLogging> */
class CMyAppLogging : public CAtEmLogging
{
public:
    virtual ~CMyAppLogging() {}

    /** \brief Application specific logging to console, e.g. print to debug output window in Visual Studio */
    virtual EC_T_VOID PrintConsole(EC_T_CHAR* szMsg) EC_OVERRIDE
    {
        /* printf console print from base class */
        CAtEmLogging::PrintConsole(szMsg);

        /* print to debug output window in Visual Studio */
